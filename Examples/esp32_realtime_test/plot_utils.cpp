...
void init_plot()
{
  mfcc_plot_buffer = new uint32_t[MBS->num_mfcc_features*MBS->num_frames*10];
...
void plot_mfcc()
{
  memcpy(mfcc_plot_buffer, mfcc_plot_buffer+2*MBS->num_mfcc_features, 4*MBS->num_mfcc_features*(10*MBS->num_frames-2));

  int x_step = 1;
  int y_step = 6;

  uint32_t* pBuffer = mfcc_plot_buffer + MBS->num_mfcc_features*(10*MBS->num_frames-2);
  int sum = 0;

  for (int i=0;i<2;i++) {
    for (int j=0;j<MBS->num_mfcc_features;j++) {
      int value = MBS->mfcc_buffer[(MBS->num_mfcc_features*(MBS->num_frames-2))+i*MBS->num_mfcc_features+j];
      uint32_t RGB  = calculate_rgb(-128, 127, value*4);
      sum += std::abs(value);
      pBuffer[i*MBS->num_mfcc_features+j] = RGB;
    }
  }
  int x_start = (screen_size_x - (MBS->num_frames*10))/2;
  x_start = (x_start>0) ? x_start:0;
  mfcc_update_counter++;
  if(mfcc_update_counter==10) {
    lcd.FillRect(0, screen_size_y/3, screen_size_x, screen_size_y/3);
    for (int i=0;i<10*MBS->num_frames;i++) {
      for (int j=0;j<MBS->num_mfcc_features;j++) {
        for (int x=0;x<x_step;x++) {
          for (int y=0;y<y_step;y++) {
            lcd.DrawPixel(x_start+i*x_step+x,100+j*y_step+y, mfcc_plot_buffer[i*MBS->num_mfcc_features+j]);
          }
        }
      }
    }
...
void plot_waveform()
{
  ...
  int stride = (MBS->audio_block_size/screen_size_x);
  ...
  {
    audio_magnitude = y_center + (int)(MBS->audio_buffer[(MBS->frame_len-MBS->frame_shift)+i*stride]/8);
