...
XMFCC::XMFCC(int num_mfcc_features, int frame_len, int mfcc_dec_bits) 
 ...
 mfcc_dec_bits(mfcc_dec_bits)
{
  ...
  //initialize FFT
  rfft = new xtensa_rfft_fast_instance_f32;
  xtensa_rfft_fast_init_f32(rfft, frame_len_padded);
...
float * XMFCC::create_dct_matrix(int32_t input_length, int32_t coefficient_count) {
  int32_t k, n;
  float * M = new float[input_length*coefficient_count];
  float normalizer;
  xtensa_sqrt_f32(2.0/(float)input_length,&normalizer);
...
void XMFCC::mfcc_compute(const int16_t * audio_data, q7_t* mfcc_out) {
  ...
  //Compute FFT
  xtensa_rfft_fast_f32(rfft, frame, buffer, 0);
  ...
  for (bin = 0; bin < NUM_FBANK_BINS; bin++) {
    ...
    for (i = first_index; i <= last_index; i++) {
      xtensa_sqrt_f32(buffer[i],&sqrt_data);
