...
void KWS::classify()
{
  ...
  // Softmax
  xtensa_softmax_q7(output,num_out_classes,output);
}
...
void KWS::average_predictions()
{
  ...
  xtensa_copy_q7((q7_t *)(predictions+num_out_classes), (q7_t *)predictions, (sliding_window_len-1)*num_out_classes);
  // add new predictions at the end
  xtensa_copy_q7((q7_t *)output, (q7_t *)(predictions+(sliding_window_len-1)*num_out_classes), num_out_classes);
  //compute averages
  int sum;
