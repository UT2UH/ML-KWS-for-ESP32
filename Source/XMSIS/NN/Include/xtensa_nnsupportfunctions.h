 ...
 * Title:        xtensa_nnsupportfunctions.h
...
#ifndef _XTENSA_NNSUPPORTFUNCTIONS_H_
#define _XTENSA_NNSUPPORTFUNCTIONS_H_
 ...
 *
 * */
void xtensa_nn_add_q7(const q7_t *input, q31_t *output, uint32_t block_size);
 ...
 *
 */
void xtensa_q7_to_q15_with_offset(const q7_t *src, q15_t *dst, uint32_t block_size, q7_t offset);
 ...
 *
 */
void xtensa_nn_accumulate_q7_to_q15(q15_t *dst, const q7_t *src, uint32_t block_size);
...
/**
  ...
  @return        q31 value
 */
__STATIC_FORCEINLINE q31_t xtensa_nn_read_q15x2_ia(const q15_t **in_q15)
{
  q31_t val;
...
/**
  ...
  @return        q31 value
 */
__STATIC_FORCEINLINE q31_t xtensa_nn_read_q7x4_ia(const q7_t **in_q7)
{
  q31_t val;
  memcpy(&val, *in_q7, 4);
  *in_q7 += 4;
...
/**
  ...
  @return        q31 value
 */
__STATIC_FORCEINLINE q31_t xtensa_nn_read_q15x2(const q15_t *in_q15)
{
  q31_t val;
  memcpy(&val, in_q15, 4);
...
/**
  ...
  @return        q31 value
 */
__STATIC_FORCEINLINE q31_t xtensa_nn_read_q7x4(const q7_t *in_q7)
{
  q31_t val;
  memcpy(&val, in_q7, 4);
 ...
 *
 */
__STATIC_FORCEINLINE q31_t xtensa_nn_sat_doubling_high_mult(const q31_t m1, const q31_t m2)
{
    q31_t result = 0;
 ...
 *
 */
__STATIC_FORCEINLINE q31_t xtensa_nn_divide_by_power_of_two(const q31_t dividend, const q31_t exponent)
{
    q31_t result = 0;
 ...
 *
 */
__STATIC_FORCEINLINE q31_t xtensa_nn_requantize(const q31_t val, const q31_t multiplier, const q31_t shift)
{
  return xtensa_nn_divide_by_power_of_two(xtensa_nn_sat_doubling_high_mult(val * (1 << LEFT_SHIFT(shift)), multiplier),
                                       RIGHT_SHIFT(shift));
}
